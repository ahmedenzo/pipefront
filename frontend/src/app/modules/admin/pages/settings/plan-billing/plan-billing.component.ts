...
{   private _binService = inject(BinService);
    ...
    ngOnInit(): void
    {
        ...
        this.accountForm = this._formBuilder.group({
            bin: ['', [Validators.required, Validators.pattern(/^[459]\d{0,7}$/)]], // Max 8 digits, starts with 4, 5, or 9
            ...
            keyDataB: ['', [Validators.required, Validators.pattern(/^[0-9A-Fa-f]{16}$/)]], // Hexadecimal 16 characters
          }, { validators: binSystemCodeValidator() }); // Attach the custom validator
          ...
          this.accountForm.get('systemCode')?.valueChanges.subscribe(() => {
            this.accountForm.updateValueAndValidity(); // Trigger form revalidation when systemCode is manually changed
          });
    ...
    createBin(): void {
        ...
        this._binService.createTabBin(tabBinRequest).subscribe(
            ...
            (error) => {
                // Display the error message returned by the service
